...
import model.DentistaAndyVC;
import model.EquipoDentalAndyVC;
...
public class DlgEquipoDental extends JDialog implements ActionListener {
	...
	void cargarDentistas() {
		String jpql = "Select d from DentistaAndyVC d";
		EntityManager manager = JPAUtil.getEntityManager();
		List<DentistaAndyVC> lstDentista = manager.createQuery(jpql, DentistaAndyVC.class).getResultList();
		for (DentistaAndyVC d : lstDentista) {
			cboDentistas.addItem(d);
		}
	}

	void listar() {
		String jpql = "Select ed,d, es.titulo FROM EquipoDentalAndyVC ed JOIN ed.idDentista d JOIN d.idEspecialidad es order by ed.nroEquipo";
		EntityManager manager = JPAUtil.getEntityManager();
		try {
			List<Object[]> resultados = manager.createQuery(jpql, Object[].class).getResultList();
			for(Object[] objects : resultados) {
				EquipoDentalAndyVC e = (EquipoDentalAndyVC)objects[0];
				DentistaAndyVC d = (DentistaAndyVC) objects[1];
				String desEspec = (String) objects[2];
				
				imprimir("***************************************");
				imprimir("Nro Equipo\t\t: " + e.getNroEquipo() );
				imprimir("Nombre\t\t\t: " + e.getNombre());
				imprimir("Costo\t\t\t: " + e.getCosto());
				imprimir("Fecha Adquisicion\t: " + e.getFechaAdquisicion());
				imprimir("Dentista\t\t: " + d.getCop() + " - " + d.getNombreCompleto());
				imprimir("Correo\t\t\t: " + d.getCorreo());
				imprimir("Especialidad\t\t: " +  desEspec);
	...
	void adicionar() {
		...
		DentistaAndyVC dent = (DentistaAndyVC) cboDentistas.getSelectedItem();
		
		EquipoDentalAndyVC eq = new EquipoDentalAndyVC(0,nom,costo,null,estado,dent);
		try {
				manager.getTransaction().begin();
				manager.persist(eq);
				manager.getTransaction().commit();
				mensaje("Equipo Dental registrado correctamente", "Exitosa", JOptionPane.INFORMATION_MESSAGE);
				limpiar();
		} catch (Exception e) {
	...
	void buscar() {
		...
		EntityManager manager = JPAUtil.getEntityManager();
		try {
			EquipoDentalAndyVC eq = manager.find(EquipoDentalAndyVC.class, nro);
			if(eq == null) {
				JOptionPane.showMessageDialog(null, "No existe ese numero de equipo", "ERROR", JOptionPane.ERROR_MESSAGE);
				return;
			}
			txtNombre.setText(eq.getNombre());
			txtCosto.setText(eq.getCosto() + "");
			txtFechaAdquisicion.setText(eq.getFechaAdquisicion()+"");
			cboDentistas.setSelectedItem(eq.getIdDentista());
	...
	void modificar() {
		...
		String estado = (String) cboEstados.getSelectedItem();
		DentistaAndyVC dent = (DentistaAndyVC) cboDentistas.getSelectedItem();
		LocalDate fecha = LocalDate.parse(txtFechaAdquisicion.getText().trim());
		
		EquipoDentalAndyVC eq = new EquipoDentalAndyVC(nro,nom,costo, fecha,estado, dent);
		try {
				manager.getTransaction().begin();
				manager.merge(eq);
				manager.getTransaction().commit();
				mensaje("Equipo Dental actualizado correctamente", "Exitosa", JOptionPane.INFORMATION_MESSAGE);
				limpiar();
	...
	void eliminar() {
		...
		try {
			...
			manager.remove(eq);
			manager.getTransaction().commit();
			mensaje("Equipo Dental eliminado correctamente", "Exitosa", JOptionPane.INFORMATION_MESSAGE);
			limpiar();
